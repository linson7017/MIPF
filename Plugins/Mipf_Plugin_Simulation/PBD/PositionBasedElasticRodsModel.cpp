...
void PositionBasedElasticRodsModel::initialize()
{
}
...
bool PositionBasedElasticRodsModel::addParticalPolyDataConstraint()
{
    ParticalPolyDataConstraint* c = new ParticalPolyDataConstraint();
    const bool res = c->initConstraint(*this);
    if (res)
    {
        m_constraints.push_back(c);
    }
...
void PBD::PositionBasedElasticRodsModel::addvtkPolyDataModel(vtkSmartPointer<vtkPolyData> polydata)
{
    m_objects.push_back(polydata);
}
