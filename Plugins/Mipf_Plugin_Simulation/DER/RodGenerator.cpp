...
RodGenerator::RodGenerator()
{ }

RodGenerator::~RodGenerator()
{ }

void RodGenerator::generateCurlyRod(const RenderObject* object, const std::vector<unsigned>& findices, Rod& o_Rod)
{
    assert(object != NULL);
    ...
    o_Rod.reset();
    o_Rod.m_object = object;
    o_Rod.m_findices = findices;
    o_Rod.m_vindices.reserve( findices.size() );
    ...
    for (unsigned i = 0; i < findices.size(); ++i)
    {
        ...
        for (unsigned j = 0; j < mesh->getNVerticesPerPrimitive(); ++j)
        {
            ...
            if (hasIdx.second)
            {
                o_Rod.m_vindices.push_back( *hasIdx.first );
            }
        }
    }

    o_Rod.m_strands.resize( o_Rod.m_vindices.size() );
...
#endif
    for (unsigned i = 0; i < o_Rod.m_vindices.size(); ++i)
    {
        unsigned idx = o_Rod.m_vindices[i];
        ...
        ElasticRod* rod = new ElasticRod(o_Rod.m_params->m_rodParams);
        generateHelicalRod(*o_Rod.m_params, p, n, u, *rod);
        o_Rod.m_strands[i] = rod;
    }

    o_Rod.initialize();
}

void RodGenerator::generateStraightRod(const RenderObject* object, const std::vector<unsigned>& findices, Rod& o_Rod)
{
    assert(object != NULL);
    assert(object->getMesh() != NULL);
    ...
    for (unsigned i = 0; i < findices.size(); ++i)
    {
       ...
       o_Rod.m_vindices.push_back(idx);
    }
...
#pragma omp parallel for
    ...
    for (unsigned i = 0; i < o_Rod.m_vindices.size(); ++i)
    {
        ...
        ElasticRod* rod = new ElasticRod(o_Rod.m_params->m_rodParams);
        generateStraightRod(*o_Rod.m_params, p, n, u, *rod);
        o_Rod.m_strands[i] = rod;
    }
...
void RodGenerator::generateHelicalRod(const RodParams& params,
...
void RodGenerator::generateStraightRod(const RodParams& params,
                                     const mg::Vec3D& p, const mg::Vec3D& n, const mg::Vec3D& u,
                                     ElasticRod& o_rod)
{
    o_rod.m_ppos.resize(params.m_nParticles);
