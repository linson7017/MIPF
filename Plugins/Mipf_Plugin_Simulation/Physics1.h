...
{
public:
	float x;									// the x value of this Vector3F
	float y;									// the y value of this Vector3F
	float z;									// the z value of this Vector3F
	...
	Vector3F(float x, float y, float z)			// Constructor that initializes this Vector3F to the intended values of x, y and z
	{
		this->x = x;
		this->y = y;
		this->z = z;
	...
	Vector3F& operator= (Vector3F v)			// operator= sets values of v to this Vector3F. example: v1 = v2 means that values of v2 are set onto v1
	{
		x = v.x;
		y = v.y;
		z = v.z;
	...
	Vector3F operator+ (Vector3F v)				// operator+ is used to add two Vector3F's. operator+ returns a new Vector3F
	{
		return Vector3F(x + v.x, y + v.y, z + v.z);
	}

	Vector3F operator- (Vector3F v)				// operator- is used to take difference of two Vector3F's. operator- returns a new Vector3F
	{
		return Vector3F(x - v.x, y - v.y, z - v.z);
	}

	Vector3F operator* (float value)			// operator* is used to scale a Vector3F by a value. This value multiplies the Vector3F's x, y and z.
	{
		return Vector3F(x * value, y * value, z * value);
	}

	Vector3F operator/ (float value)			// operator/ is used to scale a Vector3F by a value. This value divides the Vector3F's x, y and z.
	{
		return Vector3F(x / value, y / value, z / value);
	}

	Vector3F& operator+= (Vector3F v)			// operator+= is used to add another Vector3F to this Vector3F.
	{
		x += v.x;
		y += v.y;
		z += v.z;
	...
	Vector3F& operator-= (Vector3F v)			// operator-= is used to subtract another Vector3F from this Vector3F.
	{
		x -= v.x;
		y -= v.y;
		z -= v.z;
	...
	Vector3F& operator*= (float value)			// operator*= is used to scale this Vector3F by a value.
	{
		x *= value;
		y *= value;
		z *= value;
	...
	Vector3F operator- ()						// operator- is used to set this Vector3F's x, y, and z to the negative of them.
	{
		return Vector3F(-x, -y, -z);
	}

	float length()								// length() returns the length of this Vector3F
	{
		return sqrtf(x*x + y*y + z*z);
	};			   		

	void unitize()								// unitize() normalizes this Vector3F that its direction remains the same but its length is 1.
	{
	...
	Vector3F unit()								// unit() returns a new Vector3F. The returned value is a unitized version of this Vector3F.
	{
		float length = this->length();
...
public:
	...
	Vector3F pos;								// Position in space
	Vector3F vel;								// Velocity
	Vector3F force;								// Force applied on this mass at an instance
	...
	/*
	  void applyForce(Vector3F force) method is used to add external force to the mass. 
...
public:
	...
	{
		masses[0]->pos = Vector3F(0.0f, 0.0f, 0.0f);		//a mass was created and we set its position to the origin
		masses[0]->vel = Vector3F(1.0f, 0.0f, 0.0f);		//we set the mass's velocity to (1.0f, 0.0f, 0.0f)
	}
...
{
public:
	Vector3F gravitation;													//the gravitational acceleration

	MotionUnderGravitation(Vector3F gravitation) : Simulation(1, 1.0f)		//Constructor firstly constructs its super class with 1 mass and 1 kg
	{																		//Vector3F gravitation, is the gravitational acceleration
		this->gravitation = gravitation;									//set this class's gravitation
		masses[0]->pos = Vector3F(-10.0f, 0.0f, 0.0f);						//set the position of the mass
		masses[0]->vel = Vector3F(10.0f, 15.0f, 0.0f);						//set the velocity of the mass
	}
...
public:
	...
	Vector3F connectionPos;													//the arbitrary constant point that the mass is connected
	...
	MassConnectedWithSpring(float springConstant) : Simulation(1, 1.0f)		//Constructor firstly constructs its super class with 1 mass and 1 kg
	{
		...
		connectionPos = Vector3F(0.0f, -5.0f, 0.0f);						//set the connectionPos

		masses[0]->pos = connectionPos + Vector3F(10.0f, 0.0f, 0.0f);		//set the position of the mass 10 meters to the right side of the connectionPos
		masses[0]->vel = Vector3F(0.0f, 0.0f, 0.0f);						//set the velocity of the mass to zero
	}
	...
	virtual void solve()													//the spring force will be applied
	{
		for (int a = 0; a < numOfMasses; ++a)								//we will apply force to all masses (actually we have 1 mass, but we can extend it in the future)
		{
			Vector3F springVector = masses[a]->pos - connectionPos;			//find a vector from the position of the mass to the connectionPos
