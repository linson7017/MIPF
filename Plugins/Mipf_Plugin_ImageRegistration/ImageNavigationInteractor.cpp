...
ImageNavigationInteractor::ImageNavigationInteractor():m_bDragging(false), m_originMatrix(NULL), m_bInitFlag(false)
{
    m_transformMatrix.setToIdentity();
}


ImageNavigationInteractor::~ImageNavigationInteractor()
{
}

void ImageNavigationInteractor::SetDataNode(mitk::DataNode *dataNode)
{
    mitk::DataInteractor::SetDataNode(dataNode);
    Init();
}

void ImageNavigationInteractor::Init()
{
    if (!m_originMatrix)
    {
...
void ImageNavigationInteractor::SetTransformMatrix(const QMatrix4x4& matrix)
{
    m_transformMatrix = matrix;
}

QMatrix4x4 ImageNavigationInteractor::GetTransformMatrix()
{
    return m_transformMatrix;
}

void ImageNavigationInteractor::RefreshDataGeometry()
{
...
void ImageNavigationInteractor::Translate(const QVector3D& translate)
{
...
void ImageNavigationInteractor::Rotate(double angle, const QVector3D& normal)
{
...
void ImageNavigationInteractor::ConnectActionsAndFunctions()
{
...
void ImageNavigationInteractor::SelectImage(mitk::StateMachineAction *, mitk::InteractionEvent *)
{

}

void ImageNavigationInteractor::InitMove(mitk::StateMachineAction *, mitk::InteractionEvent *interactionEvent)
{
...
void ImageNavigationInteractor::MoveImage(mitk::StateMachineAction *, mitk::InteractionEvent *interactionEvent)
{
    if (m_bDragging&&m_bInitFlag)
    {
        ...
        if (positionEvent != NULL)
        {
            ...
            //emit SignalTranslateImage(QVector3D(dirVector.GetElement(0), dirVector.GetElement(1), dirVector.GetElement(2)));
...
void ImageNavigationInteractor::RotateImage(mitk::StateMachineAction *, mitk::InteractionEvent *interactionEvent)
{
    if (m_bDragging&&m_bInitFlag)
    {
        mitk::InteractionPositionEvent *positionEvent = dynamic_cast<mitk::InteractionPositionEvent *>(interactionEvent);
        if (positionEvent != NULL)
        {
           ...
           // m_SumVec = m_SumVec + dirVector;
            ...
            //emit SignalRotateImage(direction*angle, qNormal);
...
void ImageNavigationInteractor::FinishMove(mitk::StateMachineAction *, mitk::InteractionEvent *interactionEvent)
{
    if (!m_bInitFlag)
    {
        return;
