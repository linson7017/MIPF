...
class RegistrationInterfaceCommandMR : public itk::Command
{
public:
	typedef  RegistrationInterfaceCommandMR   Self;
	typedef  itk::Command                   Superclass;
	typedef  itk::SmartPointer<Self>        Pointer;
	itkNewMacro(Self);

protected:
	RegistrationInterfaceCommandMR() {};
...
class CommandIterationUpdateMR : public itk::Command
{
public:
	typedef  CommandIterationUpdateMR   Self;
	typedef  itk::Command             Superclass;
	typedef itk::SmartPointer<Self>   Pointer;
	itkNewMacro(Self);
protected:
	CommandIterationUpdateMR()
	{
...
RegistrationMR<FixedImageType, MovingImageType>::RegistrationMR()
{
}
...
template<class FixedImageType, class MovingImageType>
void RegistrationMR<FixedImageType, MovingImageType>::Start(const FixedImageType* fixedImage, const MovingImageType* movingImage, FixedImageType* resultImage, itk::Matrix<double, 4, 4>& initTransformMatrix)
{
	typedef itk::VersorRigid3DTransform<double> TransformType;
	typedef itk::VersorRigid3DTransformOptimizer OptimizerType;
	...
	CommandIterationUpdateMR::Pointer observer = CommandIterationUpdateMR::New();
	...
	registration->SetNumberOfLevels(RegistrationParameters.GetNumberOfLevels());

	typedef RegistrationInterfaceCommandMR<RegistrationType> CommandType;
...
template<class FixedImageType, class MovingImageType>
void RegistrationMR<FixedImageType, MovingImageType>::Stop()
{
	if (m_Optimizer)
	{
