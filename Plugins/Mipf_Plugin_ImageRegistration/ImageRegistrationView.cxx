    ...
    if (strcmp(szMessage, "ImageRegistration.Register") == 0)
    {
       ...
       //init matrix
        ...
        if (alignCenter)
        {
            ...
            m_initMatrix.translate(fixedImageCenter[0] - movingImageCenter[0],
       ...
       /* RegistrationMI<Float3DImageType, Float3DImageType> rm;
        ...
        emit SignalDoRegistration(itkFixedImage, itkMovingImage, m_initMatrix.inverted());
...
void ImageRegistrationView::RefreshMovingImage(QMatrix4x4& matrix)
{
    ...
    m_initMatrix = matrix*m_initMatrix;
    vtkMatrix4x4* transform = vtkMatrix4x4::New();
    vtkMatrix4x4* rm = vtkMatrix4x4::New();
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            transform->SetElement(i, j, m_initMatrix(i, j));
        }
    }
