...
void CurveProjectionVtkMapper2D::CreateExtensionCord(const mitk::Point3D& startPosition, const mitk::Vector3D& direction, const mitk::Vector3D& normal, double length, vtkPolyData* pOutput)
{
...
void CurveProjectionVtkMapper2D::CreateIntersect2D(const mitk::Point2D& intersectPosition, vtkPolyData* pOutput, double radius)
{
...
void CurveProjectionVtkMapper2D::CreateCross2D(const mitk::Point2D& crossPosition, vtkPolyData* pOutput, double size)
{
    ...
    //cursor->AllOn();
    //cursor->OutlineOff();                                                   d
    //cursor->SetFocalPoint(needleOrigin2d.GetDataPointer());
    //cursor->Update();
    //ls->m_crosshairMapper->SetInputData(cursor->GetOutput());
