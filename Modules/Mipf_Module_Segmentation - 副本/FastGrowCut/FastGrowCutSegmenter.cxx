...
//void FastGrowCut::SetSourceImage(const std::vector<short>& imSrc) 
//{
//    m_imSrc = imSrc;
//}

void FastGrowCut::SetSourceImage(vtkImageData* vtkSourceImage)
{
	m_pSourceImage = vtkSourceImage;
}

void FastGrowCut::Release()
{
    delete this;
}

//void FastGrowCut::SetSeedlImage(std::vector<unsigned char>& imSeed) 
//{
//
//    m_imSeed = imSeed;
//}

void FastGrowCut::SetSeedlImage(vtkImageData* vtkSeedImage)
{
	m_pSeedIamge = vtkSeedImage;
}
...
//
//void FastGrowCut::SetImageSize(const std::vector<long>& imSize) 
//{
//
//
//    m_imSize = imSize;
...
void FastGrowCut::Init()
{
	// Find ROI
	if (!m_bSegInitialized)
	{
		//自动根据种子点设置设置ROI
		FGC::FindVTKImageROI<unsigned char>(m_pSeedIamge, m_imROI);

		std::cout << "image ROI = [" << m_imROI[0] << "," << m_imROI[1] << "," << m_imROI[2] << ";"
			<< m_imROI[3] << "," << m_imROI[4] << "," << m_imROI[5] << "]" << std::endl;

		FGC::ExtractVTKImageROI<short>(m_pSourceImage, m_imROI, m_imSrc);
	}
	FGC::ExtractVTKImageROI<unsigned char>(m_pSeedIamge, m_imROI, m_imSeed);

	m_imSize.resize(3);
	for (int i = 0; i < 3; i++)
	{
		m_imSize[i] = m_imROI[i + 3] - m_imROI[i];
	}
}
...
//void FastGrowCut::GetLabeImage(std::vector<unsigned char>& imLab) 
//{
//
...
void FastGrowCut::GetForegroundmage(vtkImageData* vtkResultImage)
{
	long index;
	m_imLabVec.resize(m_DIMXYZ);
	index = 0;
	for (index = 0; index < m_DIMXYZ; index++) {
		m_imLabVec[index] = m_imLab[index] == 1 ? 1 : 0;
	}

	FGC::UpdateVTKImageROI<unsigned char>(m_imLabVec, m_imROI, vtkResultImage);
}


//void FastGrowCut::GetForegroundmage(std::vector<unsigned char>& imFgrd) {
//
//    long index;
