...
public:
    ...
    virtual void InitResource()
    {
        if (HasAttribute("datastorage"))
        {
            ...
            if (!GetDataStorage())
            {
                MITK_WARN << "DataStorage with id " << m_dataStorageID << " has not been initialized! ";
                return;
            }
        }
        if (HasAttribute("datamanager"))
        {
            QF::IQF_Subject* pSubject = m_pMitkDataManager->GetDataManagerSubject(GetAttribute("datamanager"));
            if (pSubject)
            {
...
protected:
    ...
    mitk::DataNode::Pointer ImportITKImage(TImageType* itkImage, const char* name, mitk::DataNode* parentNode = nullptr)
    {
