...
MitkProjectIO::~MitkProjectIO()
{
}

std::string MitkProjectIO::CreateEmptyTempDirectory()
{
    mitk::UIDGenerator uidGen("UID_", 6);

    // std::string returnValue = mitk::StandardFileLocations::GetInstance()->GetOptionDirectory() +
    // Poco::Path::separator() + "MitkProjectIOTemp" + uidGen.GetUID();
    std::string returnValue = Poco::Path::temp() + "MitkProjectIOTemp" + uidGen.GetUID();
...
mitk::DataStorage::Pointer MitkProjectIO::LoadScene(const std::string &filename,
    mitk::DataStorage *pStorage,
    bool clearStorageFirst)
{
    mitk::LocaleSwitch localeSwitch("C");

    // prepare data storage
    mitk::DataStorage::Pointer storage = pStorage;
    if (storage.IsNull())
    {
        storage = mitk::StandaloneDataStorage::New().GetPointer();
    }
    ...
    unzipper.EError += Poco::Delegate<MitkProjectIO, std::pair<const Poco::Zip::ZipLocalFileHeader, const std::string>>(
        this, &MitkProjectIO::OnUnzipError);
    unzipper.EOk += Poco::Delegate<MitkProjectIO, std::pair<const Poco::Zip::ZipLocalFileHeader, const Poco::Path>>(
        this, &MitkProjectIO::OnUnzipOk);
...
bool MitkProjectIO::SaveScene(mitk::DataStorage::SetOfObjects::ConstPointer sceneNodes,
    ...
    const std::string &filename)
{
    ...
    try
    {
        m_FailedNodes = mitk::DataStorage::SetOfObjects::New();
        m_FailedProperties = mitk::PropertyList::New();
        ...
        else
        {
            ...
            mitk::ProgressBar::GetInstance()->AddStepsToDo(sceneNodes->size());

            // find out about dependencies
            typedef std::map<mitk::DataNode *, std::string> UIDMapType;
            typedef std::map<mitk::DataNode *, std::list<std::string>> SourcesMapType;
            ...
            mitk::UIDGenerator nodeUIDGen("OBJECT_");

            for (mitk::DataStorage::SetOfObjects::const_iterator iter = sceneNodes->begin(); iter != sceneNodes->end(); ++iter)
            {
                ...
                mitk::DataStorage::SetOfObjects::ConstPointer sourceObjects = storage->GetSources(node);
            ...
            for (mitk::DataStorage::SetOfObjects::const_iterator iter = sceneNodes->begin(); iter != sceneNodes->end(); ++iter)
            {
                ...
                if (node)
                {
                    ...
                    if (mitk::BaseData *data = node->GetData())
                    {
                        ...
                        mitk::PropertyList *propertyList = data->GetPropertyList();
                    ...
                    {
                        mitk::PropertyList *propertyList = node->GetPropertyList(renderWindowName);
                        if (propertyList && !propertyList->IsEmpty())
                        {
...
TiXmlElement *MitkProjectIO::SaveBaseData(mitk::BaseData *data, const std::string &filenamehint, bool &error)
{
    assert(data);
    error = true;
    ...
    for (std::list<itk::LightObject::Pointer>::iterator iter = thingsThatCanSerializeThis.begin();
        ...
        ++iter)
    {
        if (mitk::BaseDataSerializer *serializer = dynamic_cast<mitk::BaseDataSerializer *>(iter->GetPointer()))
        {
...
TiXmlElement *MitkProjectIO::SavePropertyList(mitk::PropertyList *propertyList, const std::string &filenamehint)
{
    assert(propertyList);
    ...
    try
    {
        std::string writtenfilename = serializer->Serialize();
        element->SetAttribute("file", writtenfilename);
        mitk::PropertyList::Pointer failedProperties = serializer->GetFailedProperties();
...
const MitkProjectIO::FailedBaseDataListType *MitkProjectIO::GetFailedNodes()
{
    return m_FailedNodes.GetPointer();
}

const mitk::PropertyList *MitkProjectIO::GetFailedProperties()
{
    return m_FailedProperties;
}

void MitkProjectIO::OnUnzipError(const void * /*pSender*/,
