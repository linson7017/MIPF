...
AirwaySegmentation::AirwaySegmentation() :m_pSourceImage(NULL), m_pLabelImage(NULL)
{}

void AirwaySegmentation::Release()
{
    delete this;
}

int AirwaySegmentation::DoSegmentation()
{
	// Take care of not-supine scanned datasets
	bool flipIm = (m_pSourceImage->GetDirection()[0][0] == -1 && m_pSourceImage->GetDirection()[1][1] == -1);
	...
	if (flipIm)
	{
		...
		flipImageFilter->SetInput(m_pSourceImage);
		flipImageFilter->SetFlipAxes(axes);
		flipImageFilter->Update();
		m_pSourceImage->Graft(flipImageFilter->GetOutput());
	}

	// The different labels will be pasted into pasteImage
	OutputImageType::Pointer pasteImage = OutputImageType::New();

	pasteImage->SetRegions(m_pSourceImage->GetRequestedRegion());
	pasteImage->SetBufferedRegion(m_pSourceImage->GetBufferedRegion());
	pasteImage->SetLargestPossibleRegion(m_pSourceImage->GetLargestPossibleRegion());
	pasteImage->CopyInformation(m_pSourceImage);
	pasteImage->Allocate();
	...
	point.clear();
	point.push_back(-SeedPoint[0]);
	point.push_back(-SeedPoint[1]);
	point.push_back(SeedPoint[2]);
	seed.clear();
	...
	if (seed.size() == 1)
	{
		...
		tracheaPoint[0] = seed[tracheaIndex][0] * (-m_pSourceImage->GetDirection()[0][0]);
		tracheaPoint[1] = seed[tracheaIndex][1] * (-m_pSourceImage->GetDirection()[1][1]);
		tracheaPoint[2] = seed[tracheaIndex][2] * m_pSourceImage->GetDirection()[2][2];

		// Convert the lps physical point to index
		m_pSourceImage->TransformPhysicalPointToIndex(tracheaPoint, tracheaFiducial);
	}
	else
	{
		if (seed.size() == 0)
	...
	if (m_pSourceImage->GetLargestPossibleRegion().GetSize(2) > 200)
	{
		cropSize[2] = m_pSourceImage->GetLargestPossibleRegion().GetSize(2) - 100;
		tracheaCropStart[2] = 100;
	}
	else
	{
		cropSize[2] = m_pSourceImage->GetLargestPossibleRegion().GetSize(2) - (m_pSourceImage->GetLargestPossibleRegion().GetSize(2) / 4); //TO BE FIXED
		tracheaCropStart[2] = m_pSourceImage->GetLargestPossibleRegion().GetSize(2) / 4; // TO BE FIXED
	}

	cropSize[1] = m_pSourceImage->GetLargestPossibleRegion().GetSize(1);
	...
	ROIFilter->SetInput(m_pSourceImage);
	...
	FiducialSlice[2] = cropSize[2] - (m_pSourceImage->GetLargestPossibleRegion().GetSize(2) - tracheaFiducial[2]);
	...
	closeImage->SetRegions(m_pSourceImage->GetRequestedRegion());
	closeImage->SetBufferedRegion(m_pSourceImage->GetBufferedRegion());
	closeImage->SetLargestPossibleRegion(m_pSourceImage->GetLargestPossibleRegion());
	closeImage->CopyInformation(m_pSourceImage);
	closeImage->Allocate();
	closeImage->FillBuffer(labelValue);

	OutputImageType::SizeType sz = m_pSourceImage->GetLargestPossibleRegion().GetSize();

	if (!flipIm)
	{
		sz[0] = m_pSourceImage->GetLargestPossibleRegion().GetSize(0) - (tracheaCropStart[0] + xCarinaPosition) + 2;
	}
	else
	{
	...
	//保存最后的label图像
	m_pLabelImage->Graft(GSHoleFilling->GetOutput());

	return EXIT_SUCCESS;
}

AirwaySegmentation::OutputImageType::Pointer AirwaySegmentation::TracheaSegmentation(InputImageType::Pointer VOI,
...
typename ImageType::Pointer AirwaySegmentation::Paste(typename ImageType::Pointer sourceImage, typename ImageType::IndexType index, typename ImageType::Pointer destImage)
{
...
AirwaySegmentation::OutputImageType::Pointer AirwaySegmentation::RightLeftSegmentation(InputImageType::Pointer VOI,
	InputImageType::IndexType index,
...
void AirwaySegmentation::SetSeedPoint(const double* mitkpoint)
{
	SeedPoint[0] = mitkpoint[0];
	SeedPoint[1] = mitkpoint[1];
	SeedPoint[2] = mitkpoint[2];
}

void AirwaySegmentation::SetSourceImage( TInputImageType* const sourceimage)
{
	m_pSourceImage = (InputImageType*)sourceimage;
}

void AirwaySegmentation::SetLabelImage(TOutputImageType* labelimage)
{
	m_pLabelImage = (OutputImageType*)labelimage;
}