...
class CSFLS :   public IQF_RSSSegmentation
{
public:
    typedef CSFLS Self;
    ...
    void GetFinalMask(Float3DImageType* pInput, UShort3DImageType* pOutput, int iForegroundValue, float fUpperThreshold)
    {
        if (!pOutput||!pInput)
        {
            return;
        }
        typedef itk::BinaryThresholdImageFilter< itk::Image<float, 3>, UShort3DImageType> FilterType;
        FilterType::Pointer filter = FilterType::New();
        filter->SetInput(pInput);
        filter->SetUpperThreshold(fUpperThreshold);
        filter->SetInsideValue(iForegroundValue);
        filter->SetOutsideValue(0);
        filter->Update();
        pOutput->Graft(filter->GetOutput());
    }
